...
You run XAMPP Control Panel and activate Apache and MySQL.
Then you connect to MySQL Admin by clicking the button.
Create a user with the right privilages and set the user's name and password as it is shown in db_config.py file and create a database with the name earthquake_tracker.
Then you run the earthquake.sql file inside phpMyAdmin to create the tables and add some test data.

Now in order to see the final result of the project you run (simultaneously for the best experience) the 3 .py files (api_app.py, backend_app.py, frontend.py) in terminal.
